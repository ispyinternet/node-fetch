...
const local = new SocketServer();
const base = `unix:${local.getSocketPath()}:/`;
...
describe('node-fetch', () => {
	...
	it('should resolve into response', function() {
		...
		return fetch(url).then(res => {
			...
			expect(res.url).to.equal(url.replace(base, '127.0.0.1/'));
	...
	it('should follow redirect code 301', function() {
		...
		return fetch(url).then(res => {
			expect(res.url).to.equal(`http://127.0.0.1/inspect`);
			expect(res.status).to.equal(200);
			expect(res.ok).to.be.true;
		});
	});
	...
	it('should support redirect mode, manual flag', function() {
		...
		return fetch(url, opts).then(res => {
			expect(res.url).to.equal(url.replace(base, 'http://127.0.0.1/'));
			expect(res.status).to.equal(301);
			expect(res.headers.get('location')).to.equal(`http://127.0.0.1/inspect`);
		});
	});
...
describe('Request', function() {
	...
	it('should support wrapping Request instance', function() {
		...
		expect(r2.url).to.equal(url.replace(base, 'http://127.0.0.1/'));
	...
	it('should support parsing headers', function() {
		...
		expect(req.url).to.equal(url.replace(base, 'http://127.0.0.1/'));
	...
	it('should support clone() method', function() {
		...
		const cl = req.clone();
		expect(cl.url).to.equal(url.replace(base, 'http://127.0.0.1/'));
